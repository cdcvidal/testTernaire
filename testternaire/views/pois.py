...
from ..models import DBSession, Base, Pois
...
def getAllPois(request):
    ...
    request.response.headers.update({ 'X-Total-Count' : ''+str(len(allPois))+''})
    request.response.status_code = 200
    return data
...
def createPoi(request):
    ...
    if nbFind == nbToFind:
        try:
            ...
            DBSession().rollback()
            newPoi = DBSession.query(Pois).with_entities(Pois.id).filter(Pois.name == request.POST['name']).first()
...
def getPoi(request):
    ...
    poi = DBSession.query(Pois).get(id_)
    if poi is not None :
        data = poi.as_dict()
        request.response.status_code = 200
        return data
